...
do
  ...
  vim.keymap.set({ 'n', 'i' }, '<C-s>', '<cmd>write<CR>', { desc = 'Save and format' })
  ...
  -- Font zoom with Ctrl + / Ctrl - (works in GUI + kitty terminal) + Space z
  local orig_guifont_size = 14
  local orig_kitty_size = 12.0

  local function get_font_size()
    local font = vim.o.guifont
    local size = font:match(':h(%d+%.?%d*)')
    return tonumber(size) or orig_guifont_size
  end

  local function set_font_size(size)
    local font = vim.o.guifont
    local family = font:match('^([^:]+)') or 'JetBrainsMono NF'
    vim.o.guifont = family .. ':h' .. size
    vim.notify('Font size: ' .. size, vim.log.levels.INFO)
  end

  local function is_kitty()
    return vim.fn.executable('kitty') == 1 and (vim.env.KITTY_LISTEN_ON ~= nil or vim.env.TERM == 'xterm-kitty' or vim.env.KITTY_PID ~= nil)
  end

  local function zoom_in()
    if vim.g.neovide then
      vim.g.neovide_scale_factor = (vim.g.neovide_scale_factor or 1) + 0.1
      vim.notify(string.format('Zoom: %.1f', vim.g.neovide_scale_factor))
    elseif is_kitty() then
      vim.system({ 'kitty', '@', 'set-font-size', '--all', '+1' }, { text = true }, function(obj)
        if obj.code ~= 0 then
          vim.schedule(function() set_font_size(get_font_size() + 1) end)
        end
      end)
    else
  ...
  local function zoom_out()
    ...
    elseif is_kitty() then
      vim.system({ 'kitty', '@', 'set-font-size', '--all', '--', '-1' }, { text = true }, function(obj)
        if obj.code ~= 0 then
          vim.schedule(function() set_font_size(math.max(6, get_font_size() - 1)) end)
        end
      end)
    else
      set_font_size(math.max(6, get_font_size() - 1))
    end
  end
  ...
  local function zoom_reset()
    if vim.g.neovide then
      ...
      vim.notify('Zoom reset: 1.0')
    elseif is_kitty() then
      vim.system({ 'kitty', '@', 'set-font-size', '--all', '0' }, { text = true }, function(obj)
        if obj.code ~= 0 then
          vim.schedule(function() set_font_size(orig_guifont_size) end)
        end
      end)
    else
      set_font_size(orig_guifont_size)
    end
  end

  -- Ctrl + / Ctrl -  (terminal + GUI) — cover all keycodes for +/-
  for _, mode in ipairs({ 'n', 'i', 'v', 'x', 't' }) do
    vim.keymap.set(mode, '<C-=>', zoom_in, { desc = 'Zoom In' })
    vim.keymap.set(mode, '<C-+>', zoom_in, { desc = 'Zoom In' })
    vim.keymap.set(mode, '<C-kPlus>', zoom_in, { desc = 'Zoom In' })
    vim.keymap.set(mode, '<C-->', zoom_out, { desc = 'Zoom Out' })
    vim.keymap.set(mode, '<C-_>', zoom_out, { desc = 'Zoom Out' })
    vim.keymap.set(mode, '<C-kMinus>', zoom_out, { desc = 'Zoom Out' })
    vim.keymap.set(mode, '<C-0>', zoom_reset, { desc = 'Zoom Reset' })
...
do
  ...
  local servers = {
    ...
    -- See: https://github.com/neovim/nvim-lspconfig
     ...
     -- V language support (v-analyzer via Mason or ~/.config/v-analyzer)
     v_analyzer = (function()
       local candidates = {
         vim.fn.stdpath 'data' .. '/mason/bin/v-analyzer',
         vim.fn.expand '~/.config/v-analyzer/bin/v-analyzer',
         'v-analyzer',
       }
       local cmd = { 'v-analyzer' }
       for _, c in ipairs(candidates) do
         if c == 'v-analyzer' or vim.fn.executable(c) == 1 then
           cmd = { c }
           break
         end
       end
...
do
  ...
  require('blink.cmp').setup {
    ...
    --
    -- Prefer the Rust matcher; fall back to Lua if the binary is unavailable.
